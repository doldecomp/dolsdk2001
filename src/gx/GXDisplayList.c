...
void GXBeginDisplayList(void *list, u32 size)
{
    ...
    ASSERTMSGLINE(__FILE__, 0x7D, !gx->inDispList, "GXBeginDisplayList: display list already in progress");
    ASSERTMSGLINE(__FILE__, 0x7E, (size & 0x1F) == 0, "GXBeginDisplayList: size is not 32 byte aligned");
    ASSERTMSGLINE(__FILE__, 0x7F, ((u32)list & 0x1F) == 0, "GXBeginDisplayList: list is not 32 byte aligned");
    if (gx->dirtyState != 0) {
        __GXSetDirtyState();
    }
...
unsigned long GXEndDisplayList(void)
{
    ...
    ASSERTMSGLINE(__FILE__, 0xB6, gx->inDispList == TRUE, "GXEndDisplayList: no display list in progress");
    if (gx->dirtyState != 0) {
        __GXSetDirtyState();
    }
    reg = __piReg[5];
    ...
    ASSERTMSGLINE(__FILE__, 0xC3, !ov, "GXEndDisplayList: display list commands overflowed buffer");
...
void GXCallDisplayList(void *list, u32 nbytes)
{
    ...
    ASSERTMSGLINE(__FILE__, 0xED, !gx->inDispList, "GXCallDisplayList: display list already in progress");
    ASSERTMSGLINE(__FILE__, 0xEE, (nbytes & 0x1F) == 0, "GXCallDisplayList: nbytes is not 32 byte aligned");
    ASSERTMSGLINE(__FILE__, 0xEF, ((u32)list & 0x1F) == 0, "GXCallDisplayList: list is not 32 byte aligned");
