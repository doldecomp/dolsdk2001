...
#endif
    ASSERTLINE(0x15A+LINE_OFFSET, !GXOverflowSuspendInProgress);
...
#endif
    ASSERTLINE(0x184+LINE_OFFSET, GXOverflowSuspendInProgress);
...
void GXInitFifoBase(GXFifoObj *fifo, void *base, u32 size)
{
    ...
    ASSERTMSGLINE(0x1FF+LINE_OFFSET, realFifo != CPUFifo,     "GXInitFifoBase: fifo is attached to CPU");
    ASSERTMSGLINE(0x201+LINE_OFFSET, realFifo != GPFifo,      "GXInitFifoBase: fifo is attached to GP");
    ASSERTMSGLINE(0x203+LINE_OFFSET, ((u32)base & 0x1F) == 0, "GXInitFifoBase: base must be 32B aligned");
    ASSERTMSGLINE(0x205+LINE_OFFSET, base != NULL,            "GXInitFifoBase: base pointer is NULL");
    ASSERTMSGLINE(0x207+LINE_OFFSET, (size & 0x1F) == 0,      "GXInitFifoBase: size must be 32B aligned");
    ASSERTMSGLINE(0x209+LINE_OFFSET, size >= 0x10000,         "GXInitFifoBase: fifo is not large enough");
...
void GXInitFifoPtrs(GXFifoObj *fifo, void *readPtr, void *writePtr)
{
    ...
    ASSERTMSGLINE(0x231+LINE_OFFSET, realFifo != CPUFifo,         "GXInitFifoPtrs: fifo is attached to CPU");
    ASSERTMSGLINE(0x233+LINE_OFFSET, realFifo != GPFifo,          "GXInitFifoPtrs: fifo is attached to GP");
    ASSERTMSGLINE(0x235+LINE_OFFSET, ((u32)readPtr & 0x1F) == 0,  "GXInitFifoPtrs: readPtr not 32B aligned");
    ASSERTMSGLINE(0x237+LINE_OFFSET, ((u32)writePtr & 0x1F) == 0, "GXInitFifoPtrs: writePtr not 32B aligned");
    ASSERTMSGLINE(0x23A+LINE_OFFSET, realFifo->base <= readPtr && readPtr < realFifo->top,   "GXInitFifoPtrs: readPtr not in fifo range");
    ASSERTMSGLINE(0x23D+LINE_OFFSET, realFifo->base <= writePtr && writePtr < realFifo->top, "GXInitFifoPtrs: writePtr not in fifo range");
...
void GXInitFifoLimits(GXFifoObj *fifo, u32 hiWatermark, u32 loWatermark)
{
    ...
    ASSERTMSGLINE(0x262+LINE_OFFSET, realFifo != GPFifo,        "GXInitFifoLimits: fifo is attached to GP");
    ASSERTMSGLINE(0x264+LINE_OFFSET, (hiWatermark & 0x1F) == 0, "GXInitFifoLimits: hiWatermark not 32B aligned");
    ASSERTMSGLINE(0x266+LINE_OFFSET, (loWatermark & 0x1F) == 0, "GXInitFifoLimits: loWatermark not 32B aligned");
    ASSERTMSGLINE(0x268+LINE_OFFSET, hiWatermark < realFifo->top - realFifo->base, "GXInitFifoLimits: hiWatermark too large");
    ASSERTMSGLINE(0x26A+LINE_OFFSET, loWatermark < hiWatermark, "GXInitFifoLimits: hiWatermark below lo watermark");
...
{
    struct __GXFifoObj *realFifo = (struct __GXFifoObj *)fifo;
    ASSERTMSGLINE(0x321+LINE_OFFSET, realFifo == CPUFifo, "GXSaveCPUFifo: fifo is not attached to CPU");
...
void GXSaveGPFifo(GXFifoObj *fifo)
{
    ...
    ASSERTMSGLINE(0x36A+LINE_OFFSET, realFifo == GPFifo, "GXSaveGPFifo: fifo is not attached to GP");
    cpStatus = __cpReg[0];
    readIdle = GET_REG_FIELD(cpStatus, 1, 2);
    ASSERTMSGLINE(0x371+LINE_OFFSET, readIdle, "GXSaveGPFifo: GP is not idle");
...
{
    SET_REG_FIELD(0x4B0+LINE_OFFSET, gx->cpEnable, 1, 4, (en != 0) ? 1 : 0);
    __cpReg[1] = gx->cpEnable;
}

static void __GXWriteFifoIntEnable(u8 hiWatermarkEn, u8 loWatermarkEn)
{
    SET_REG_FIELD(0x4C6+LINE_OFFSET, gx->cpEnable, 1, 2, hiWatermarkEn);
    SET_REG_FIELD(0x4C7+LINE_OFFSET, gx->cpEnable, 1, 3, loWatermarkEn);
    __cpReg[1] = gx->cpEnable;
}

static void __GXWriteFifoIntReset(u8 hiWatermarkClr, u8 loWatermarkClr)
{
    SET_REG_FIELD(0x4DE+LINE_OFFSET, gx->cpClr, 1, 0, hiWatermarkClr);
    SET_REG_FIELD(0x4DF+LINE_OFFSET, gx->cpClr, 1, 1, loWatermarkClr);
    __cpReg[2] = gx->cpClr;
}
...
OSThread *GXSetCurrentGXThread(void)
{
    ...
    ASSERTMSGLINE(0x532+LINE_OFFSET, !GXOverflowSuspendInProgress, "GXSetCurrentGXThread: Two threads cannot generate GX commands at the same time!");
    __GXCurrentThread = OSGetCurrentThread();
    OSRestoreInterrupts(enabled);
    return prev;
}
