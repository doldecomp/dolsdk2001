...
GXFifoObj *GXInit(void *base, u32 size)
{
    ...
    for (i = 0; i < 16; ++i) {
        ...
        SET_REG_FIELD(0x2F2+LINE_OFFSET, gx->tevc[i], 8, 24, 0xC0 + i * 2);
        SET_REG_FIELD(0x2F3+LINE_OFFSET, gx->teva[i], 8, 24, 0xC1 + i * 2);
        SET_REG_FIELD(0x2F5+LINE_OFFSET, gx->tevKsel[i / 2], 8, 24, 0xF6 + i / 2);
        SET_REG_FIELD(0x2F7+LINE_OFFSET, gx->tref[i / 2], 8, 24, 0x28 + i / 2);
    }
    gx->iref = 0;
    ...
    for (i = 0; i < 8; ++i) {
        gx->suTs0[i] = 0;
        gx->suTs1[i] = 0;
        SET_REG_FIELD(0x300+LINE_OFFSET, gx->suTs0[i], 8, 24, 0x30 + i * 2);
        SET_REG_FIELD(0x301+LINE_OFFSET, gx->suTs1[i], 8, 24, 0x31 + i * 2);
    }
...
void __GXInitGX(void)
{
    ...
    case VI_MPAL: rmode = &GXMpal480IntDf; break;
    default:
        ASSERTMSGLINE(0x3FE, 0, "GXInit: invalid TV format");
        rmode = &GXNtsc480IntDf;
        break;
    }
    GXSetCopyClear(clear, 0xFFFFFF);
